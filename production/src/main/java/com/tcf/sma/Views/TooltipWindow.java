...
public class TooltipWindow {
    ...
    private static TooltipWindow tooltipWindow = null;

    public static TooltipWindow getInstance(Context ctx,int id)
    {
        if (tooltipWindow == null)
            tooltipWindow = new TooltipWindow(ctx,id);

        return tooltipWindow;
    }


    public TooltipWindow(Context ctx, int id) {
        this.ctx = ctx;
        tipWindow = new PopupWindow(ctx);

        inflater = (LayoutInflater) ctx.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        contentView = inflater.inflate(R.layout.tooltip_layout, null);
        TextView tv_text = contentView.findViewById(R.id.tooltip_text);
        tv_text.setText(id);
    }
